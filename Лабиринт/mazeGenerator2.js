function mapGen(canvas, w, h, steps, mazes) {
    ...
    function character(dx, dy) {
        ...
        0 == pixel.data[0] && 0 == pixel.data[1] && 0 == pixel.data[2] && 255 == pixel.data[3] ? dx = dy = 0 : document.querySelector("#step").innerHTML = Math.floor(document.querySelector("#step").innerHTML) + 1;
    ...
    for (cr_l = 0; cr_l < h; cr_l++) {
        ...
        for (i = 0; i < w; i++) {
            var p = l = 0;
            for (j = 0; j < w; j++) a[i] == a[j] && 0 == b[j] ? p++ : l++;
